...
public class AI extends Paddle {
    ...
    public AI(PingPongGame game, Color color, int x, int height, int width, Ball ball, boolean onRight, int minX,
            int maxX, double skill) {
    ...
    public void move() {
        ...
        if (skill == 0) {
            if (Math.abs(this.getY() + this.getHeight() / 2 - (ball.getY() + ball.getSize() / 2)) > 10) {
                if (this.getY() + this.getHeight() / 2 < ball.getY() + ball.getSize() / 2) {
                    this.setYVel(3);
                } else {
                    this.setYVel(-3);
                }
        ...
        if (Math.abs(tmpVX) >= 0.00001)
            while (!est && amo < max) {
                if (onRight) {
                    if (tmpX >= this.getX() - ball.getSize()) {
                        estY = tmpY;
                        est = true;
                    }
                } else {
                ...
                if (tmpY > game.getHeight() - ball.getSize()) {
                    tmpY = game.getHeight() - ball.getSize();
                    tmpVY = -tmpVY;
                }
